...
/**
 * A path on GCS. Contains information about the bucket and blob name (if applicable). A path is of
 * the form gs://bucket/name.
 */
public class GCSPath {
  ...
  public static GCSPath from(String path) {
    ...
    if (!Pattern.matches("[a-z0-9-_.]+", bucket)) {
      throw new IllegalArgumentException(
          String.format(
              "Invalid bucket name in path '%s'. Bucket name should only contain lower case"
                  + " alphanumeric, '-'. '_' and '.'. Please follow GCS naming convention: "
